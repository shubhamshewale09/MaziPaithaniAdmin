...
import { MIN_LOADING_TIME, pagesArray, truncateHtml, truncateText } from "../../../Utils/Utils";
...
import { ChangeEventStatus, GetEventList } from "../../../services/Event/EventApi";
...
const ListComponent = () => {
    ...
    const [appliedFilter, setAppliedFilter] = useState({
        ...
        toDate: null,
        userKeyID: userKeyID,
        eventKeyID: state.eventKeyID
        // userKeyID: "CFE9DCF2-A0D2-4918-B52A-C1C4951A99F1" //testing purpose
    });
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetEventList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeEventStatus(modelRequestData.eventKeyID, modelRequestData.appLangID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Title", "Description", "Language", "Event Date", "Address", "Event Day", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                        ...
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 text-gray-700">{u.eventTitle}</td>
                                    <td className="px-4 py-2 text-gray-700">
                                        {u.description ? parse(truncateHtml(u.description, 20)) : "-"}
                                    </td>
                                    <td className="px-4 py-2 text-gray-700">{u.languageName || "-"}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.eventDate || "-"}</td>
                                    <td className="px-4 py-2 text-gray-700"> <Tooltip title={u.address}>{u.address ? truncateText(u.address, 15) : "-"} </Tooltip></td>
                                    <td className="px-4 py-2">{u.eventDay}</td>
            ...
            {/* User Form Modal */}
            {
                modalOpen && (
                    <AddUpdateForm
                        ...
                        keyID={state.eventKeyID} //parent key id for add update
