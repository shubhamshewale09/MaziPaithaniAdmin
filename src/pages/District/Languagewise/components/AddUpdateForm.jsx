...
import { GetAllLanguageLookUpList, GetStateLookupList } from "../../../../services/List/AllLookupList";
...
const AddUpdateForm = ({ onClose, modelRequestData, existingLangIDs }) => {
    ...
    useEffect(() => {
        const fetchLookupData = async () => {
            try {
                ...
                // Map state
                const mappedStates = mapToSelectOptions(
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await AddUpdateDistrict({ ...formData, userKeyID });
    ...
    const GetModelData = async (KeyID, appLangID) => {
        const data = await GetDistrictModel(KeyID, appLangID);
        if (data.statusCode === 200) {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
            <div className="bg-white rounded-2xl flex flex-col shadow-lg text-gray-700 text-base w-full sm:w-80 md:w-96 lg:w-96 xl:w-96 max-h-[90vh]">
                ...
                <div className="bg-background px-6 py-4 border-b border-gray-200 sticky top-0 z-10 rounded-t-2xl">
                    <h2 className="text-xl font-semibold">{modelRequestData?.Action == "Update" ? "Update District" : "Add District"}</h2>
                </div>
                ...
                >
                    ...
                    <div>
                        <label className="block font-medium">
                            District Title <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="text"
                            name="districtName"
                            ...
                            onChange={(e) => {
                                ...
                                setFormData({ ...formData, districtName: cleanedValue });
                            }}
                            onBlur={(e) => {
                                const trimmedValue = trimOnBlur(e.target.value);
                                setFormData({ ...formData, districtName: trimmedValue });
                            }}
                            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 ${errors.districtName ? "border-red-500 focus:ring-red-500" : "focus:ring-secondary"}`}
                        />
                        {errors.districtName && <p className="text-red-500 text-sm mt-1">{errors.districtName}</p>}
                    </div>
                </form>
