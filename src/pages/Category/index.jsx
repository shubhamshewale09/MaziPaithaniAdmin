...
import { ChangeCategoryStatus, GetCategoryList } from "../../services/Category/CategoryApi";
...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetCategoryList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeCategoryStatus(modelRequestData.ccKeyID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Category Name", "Complaint Type", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                        ...
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 text-gray-700">{u.complaintCategory}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.complaintType}</td>
                                        ...
                                        <div className="flex gap-2 justify-center">
                                            ...
                                            <Tooltip title="Add Languagewise">
                                                <button
                                                    className="text-blue-500 hover:text-blue-700"
                                                    onClick={() =>
                                                        navigate("/category-languagewise", {
                                                            state: { ccKeyID: u.ccKeyID }
                                                        })
                                                    }
                                                >
                                                    <FaLanguage />
