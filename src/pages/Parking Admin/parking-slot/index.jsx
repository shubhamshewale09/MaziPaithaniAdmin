...
const ListComponent = () => {
    ...
    const [selectedFilters, setSelectedFilters] = useState({
        isAdminApproved: filterOptions[0],      // first filter
        isPaidParking: filterOptions2[0],       // second filter
        isGovParking: filterOptions3[0], // third filter
    });
    ...
    const [appliedFilter, setAppliedFilter] = useState({
        ...
        userKeyID: state?.fromDashboard ? userKeyID : state?.userKeyIDForUpdate,
        parkingOwnerID: state?.fromDashboard ? null : state?.userID,
        isAdminApproved: state?.fromDashboard ? state?.status : null,
        isPaidParking: state?.fromDashboard ? state?.isPaidParking : null,
        isGovParking: state?.fromDashboard ? state?.isGovParking : null,
    });
    ...
    useEffect(() => {
        ...
        setSelectedFilters({
            isAdminApproved:
                filterOptions.find(o => o.value === state?.status) || filterOptions[0],

            isPaidParking:
                filterOptions2.find(o => o.value === state?.isPaidParking) || filterOptions2[0],

            isGovParking:
                filterOptions3.find(o => o.value === state?.isGovParking) || filterOptions3[0],
        });
    }, [state]);
          ...
          focus:outline-none focus:ring-1 focus:ring-secondary text-sm"
                    ...
                    <div className="w-[120px] md:w-48 text-sm">
                        <Select
                            ...
                            isClearable
                            placeholder="Status"
                            value={selectedFilters.isAdminApproved}
                            onChange={(option) => {
                                const selected = option || filterOptions[0]; // fallback to 'All'

                                // Update the object state with full { value, label }
                                setSelectedFilters(prev => ({ ...prev, isAdminApproved: selected }));

                                // Update appliedFilter with the value only
                                setAppliedFilter(prev => ({
                                    ...prev,
                                    isAdminApproved: selected.value,  // this will be null / "Approved" / "Pending" / "Rejected"
                                    pageNo: 0
                                }));
                            }}
                    ...
                    <div className="w-[120px] md:w-48 text-sm">
                        <Select
                            ...
                            value={selectedFilters.isPaidParking}
                            onChange={(option) => {
                                const selected = option || filterOptions2[0]; // fallback to 'All'

                                // Update the object state with full { value, label }
                                setSelectedFilters(prev => ({ ...prev, isPaidParking: selected }));

                                // Update appliedFilter with the value only
                                setAppliedFilter(prev => ({
                                    ...prev,
                                    isPaidParking: selected.value,  // this will be null / "Approved" / "Pending" / "Rejected"
                                    pageNo: 0
                                }));
                            }}
                            // classNamePrefix="react-select"
                    ...
                    <div className="w-[120px] md:w-48 text-sm">
                        <Select
                            ...
                            value={selectedFilters.isGovParking}
                            onChange={(option) => {
                                const selected = option || filterOptions3[0]; // fallback to 'All'

                                // Update the object state with full { value, label }
                                setSelectedFilters(prev => ({ ...prev, isGovParking: selected }));

                                // Update appliedFilter with the value only
                                setAppliedFilter(prev => ({
                                    ...prev,
                                    isGovParking: selected.value,  // this will be null / "Approved" / "Pending" / "Rejected"
                                    pageNo: 0
                                }));
                            }}
                            // classNamePrefix="react-select"
