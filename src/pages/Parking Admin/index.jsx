...
import { FaCar, FaEdit, FaUser } from "react-icons/fa";
import { Eye, EyeOff } from "lucide-react";
import { Tooltip } from "@mui/material";
import { useNavigate } from "react-router-dom";

import UserForm from "./components/UserForm";
import { ChangeUserStatus, GetUserList } from "../../services/User/UserApi";
...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetUserList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeUserStatus(modelRequestData.userKeyIDForUpdate);
    ...
    return (
        <div className="h-full flex flex-col rounded-xl border border-gray-200 bg-white shadow">
            {/* Header */}
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Name", "Email", "Mobile No", "Address", "Password", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                        ...
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 text-gray-700">{u.emailID || "-"}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.mobileNo || "-"}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.address}</td>
                                    ...
                                    <td className="px-4 py-2 text-gray-700">
                                        {u.password ? (
                                            <div className="flex items-center justify-center gap-2">
                                                <span>
                                                    {visiblePasswords[u.userKeyIDForUpdate]
                                                        ? u.password
                                                        : "***"}
                                                </span>
                                                <button
                                                    type="button"
                                                    onClick={() => togglePassword(u.userKeyIDForUpdate)}
                                                    className="text-gray-500 hover:text-gray-700"
                                                >
                                                    {visiblePasswords[u.userKeyIDForUpdate] ? (
                                                        <EyeOff size={16} />
                                                    ) : (
                                                        <Eye size={16} style={{ marginBottom: "5px" }} />
                                                    )}
                                                </button>
                                            </div>
                                        ) : (
                                        ...
                                        <div className="flex gap-2 justify-center">
                                            ...
                                            <Tooltip title="Add Parking">
                                                <button
                                                    className="text-blue-500 hover:text-blue-700"
                                                    onClick={() =>
                                                        navigate("/users/parking-slot", {
                                                            state: {
                                                                userID: u.userID, // for list
                                                                userKeyIDForUpdate: u.userKeyIDForUpdate //for add update
                                                            },
                                                        })
                                                    }
