...
import { GetAllLanguageLookUpList, GetServiceAggregatorLookupList, GetServiceTypeLookupList } from "../../../../services/List/AllLookupList";
...
const AddUpdateForm = ({ onClose, modelRequestData, KeyID, existingLangIDs }) => {
    ...
    useEffect(() => {
        const fetchLookupData = async () => {
            try {
                ...
                // Map state
                const mappedService = mapToSelectOptions(
                ...
                //   Update state ONCE
                setLookupList({
                    serviceTypeLookupList: mappedService,
    ...
    const GetModelData = async (KeyID, appLangID) => {
        const data = await GetServiceAggregatorModel(KeyID, appLangID);
        if (data.statusCode === 200) {
