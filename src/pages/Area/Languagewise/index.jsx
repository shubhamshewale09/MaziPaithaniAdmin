...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetAreaList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeAreaStatus(modelRequestData.areaKeyID, modelRequestData.appLangID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Name", "Language", "Date", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
            ...
            {/* User Form Modal */}
            {
                modalOpen && (
                    <AddUpdateForm
                        ...
                        keyID={state.areaKeyID} //parent key id for add update
