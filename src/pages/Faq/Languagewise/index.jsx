...
import { ChangeFAQStatus, GetFAQList } from "../../../services/Faq/FaqApi";
...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetFAQList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeFAQStatus(modelRequestData.faqKeyID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Question", "Answer", "Language", "Date", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                        ...
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 text-gray-700"> <Tooltip title={u.question}>{u.question ? parse(truncateText(u.question, 20)) : "-"}</Tooltip></td>
                                    <td className="px-4 py-2 text-gray-700">
                                        <Tooltip title={u.answer}>   {u.answer ? parse(truncateText(u.answer, 20)) : "-"}</Tooltip>
                                    </td>
                                    <td className="px-4 py-2 text-gray-700">  {u.languageName || "-"} </td>
                                    {/* <td className="px-4 py-2 text-gray-700">{u.questionBy || "-"}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.answerBy || "-"}</td> */}
                                    <td className="px-4 py-2 text-gray-700">{u.questionOnDate || "-"}</td>
