...
const ListComponent = () => {
    ...
    const [appliedFilter, setAppliedFilter] = useState({
        ...
        loastAndFoundKeyID: state.loastAndFoundKeyID
    });
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                const res = await GetComplaintCategories(2);
    ...
    return (
        <div className="bg-background h-full flex flex-col rounded-xl border border-gray-200 shadow">
          ...
          focus:outline-none focus:ring-1 focus:ring-secondary text-sm"
                ...
                {/* Right side: Add Complaint button */}
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Name", "Mobile", "Address", "Description", "Date","Language", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                                    ...
                                    <td className="px-4 py-2">
                                        <div className="cursor-pointer flex items-center space-x-2 justify-center">
                                            <Tooltip title="Change Status">
                                                <span
                                                    className={`cursor-pointer text-12 font-medium ${u.status === "Open" ? "text-green-600" : "text-red-600"
                                                        }`}
                                                    onClick={() => {
                                                        setModelRequestData({
                                                            ...modelRequestData,
                                                            loastAndFoundKeyID: u.loastAndFoundKeyID,
                                                            status: u.status === "Open" ? "Open" : "Closed",
                                                        });
