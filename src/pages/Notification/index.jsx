...
import { ChangeNotificationStatus, GetNotificationList } from "../../services/Notification/NotificationApi";
...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetNotificationList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeNotificationStatus(modelRequestData.notificationKeyID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Title", "Message", "Action"].map((h) => (
                                <th
                                    key={h}
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                <tr
                                    key={u.notificationKeyID}
                                    className="hover:bg-gray-50 transition-colors text-center"
                                >
                                    <td className="px-4 py-2 text-gray-600">
                                        {(currentPage - 1) * pageSize + idx + 1}
                                    </td>
                                    <td className="px-4 py-2 text-gray-700">{u.notificationTitle}</td>
                                    <td className="px-4 py-2 text-gray-700"> <Tooltip title={u.message}>{u.message ? truncateText(u.message, 15) : "-"} </Tooltip></td>
                                            ...
                                            <Tooltip title="Change Status">
                                                <span
                                                    ...
                                                    onClick={() => {
                                                        setModelRequestData({
                                                            ...modelRequestData,
                                                            notificationKeyID: u.notificationKeyID,
