...
const AddUpdateForm = ({ onClose, modelRequestData }) => {
    ...
    const GetModelData = async (KeyID) => {
        const data = await GetStateModel(KeyID);
        if (data.statusCode === 200) {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
            <div className="bg-white rounded-2xl flex flex-col shadow-lg text-gray-700 text-base w-full sm:w-80 md:w-96 lg:w-96 xl:w-96 max-h-[90vh]">
                ...
                <div className="bg-background px-6 py-4 border-b border-gray-200 sticky top-0 z-10 rounded-t-2xl">
                    <h2 className="text-xl font-semibold">{modelRequestData?.Action == "Update" ? "Update Group" : "Add Group"}</h2>
                </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => {
                                ...
                                setFormData({ ...formData, groupName: cleanedValue });
                            }}
                            onBlur={(e) => {
                                const trimmedValue = trimOnBlur(e.target.value);
                                setFormData({ ...formData, groupName: trimmedValue });
                            }}
                            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 ${errors.groupName ? "border-red-500 focus:ring-red-500" : "focus:ring-secondary"
                                }`}
                        />
