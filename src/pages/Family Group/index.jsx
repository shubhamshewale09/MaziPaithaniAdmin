...
import { ChangeGroupStatus, GetFamilyGroupList } from "../../services/Family Group/FamilyGroupApi";
...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetFamilyGroupList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeGroupStatus(modelRequestData.familyGroupKeyID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Group Name", "Group ID", "Admin Name", "Mobile No", "Total Group Members"].map((h) => (
                                <th
                                    key={h}
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                <tr
                                    key={u.familyGroupKeyID}
                                    className="hover:bg-gray-50 transition-colors text-center"
                                >
                                    <td className="px-4 py-2 text-gray-600">
                                        {(currentPage - 1) * pageSize + idx + 1}
                                    </td>
                                    <td className="px-4 py-2 text-gray-700">{u.groupName}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.groupID}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.groupAdminName}</td>
                                    <td className="px-4 py-2 text-gray-700">{u.mobileNo}</td>
                                    <td className="px-4 py-2 text-center">
                                        <span
                                            className="text-blue-600 hover:text-blue-800 hover:underline cursor-pointer"
                                            onClick={() =>
                                                navigate("/family-safety/family-group/members", {
                                                    state: { familyGroupKeyID: u.familyGroupKeyID },
                                                })
                                            }
                                        >
                                            {u.totalGroupMembers}
                                            ...
                                            <Tooltip title="Change Status">
                                                <span
                                                    ...
                                                    onClick={() => {
                                                        setModelRequestData({
                                                            ...modelRequestData,
                                                            familyGroupKeyID: u.familyGroupKeyID,
                                        ...
                                        <div className="flex gap-2 justify-center">
                                            <Tooltip title="View Members">
                                                <button
                                                    ...
                                                    onClick={() => navigate('/family-safety/family-group/members', { state: { familyGroupKeyID: u.familyGroupKeyID } })}
                                                >
                                                    <FaPeopleLine />
                                                </button>
                                            </Tooltip>
