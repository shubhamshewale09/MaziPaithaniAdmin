...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetServiceTypeList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeServiceTypeStatus(modelRequestData.serviceTypeKeyID);
    ...
    return (
        <div className="bg-background h-full flex flex-col rounded-xl border border-gray-200 shadow">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-2">
                ...
                <div className="w-fit">
                    <Tooltip title="Add Govt. Service Type">
                        <button
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Govt. Service Type", "Date", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                <tr
                                    key={u.serviceTypeKeyID}
                                    className="hover:bg-gray-50 transition-colors text-center"
                                >
                                    <td className="px-4 py-2 text-gray-600">
                                        {(currentPage - 1) * pageSize + idx + 1}
                                    </td>
                                    <td className="px-4 py-2 text-gray-700">{u.serviceType}</td>
                                            ...
                                            <Tooltip title="Change Status">
                                                <span
                                                    ...
                                                    onClick={() => {
                                                        setModelRequestData({
                                                            ...modelRequestData,
                                                            serviceTypeKeyID: u.serviceTypeKeyID,
                                        ...
                                        <div className="flex gap-2 justify-center">
                                            ...
                                            <Tooltip title="Add Language-wise">
                                                <button
                                                    className="text-blue-500 hover:text-blue-700"
                                                    onClick={() =>
                                                        navigate("/service-type/languagewise", {
                                                            state: {
                                                                serviceTypeKeyID: u.serviceTypeKeyID,
