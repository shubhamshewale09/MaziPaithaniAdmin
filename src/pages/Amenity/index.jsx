...
import { ChangeAreaStatus, GetAreaList } from "../../services/Area/AreaApi";
import BackButton from "../../components/custom/BackButton";
import { ChangeAmenityStatus, GetAmenityList } from "../../services/Amenity/AmenityApi";
...
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetAmenityList(appliedFilter);
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeAmenityStatus(modelRequestData.amenityKeyID);
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Icon", "Name", "Date", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                        ...
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 text-gray-700">{u.amenityName}</td>
                                        ...
                                        <div className="flex gap-2 justify-center">
                                            ...
                                            <Tooltip title="Add Languagewise">
                                                <button
                                                    className="text-blue-500 hover:text-blue-700"
                                                    onClick={() =>
                                                        navigate("/amenity/languagewise", {
                                                            state: { amenityKeyID: u.amenityKeyID }
                                                        })
                                                    }
                                                >
                                                    <FaLanguage />
