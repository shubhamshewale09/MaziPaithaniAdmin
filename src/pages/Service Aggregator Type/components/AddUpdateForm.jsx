...
import { AddUpdateServiceType, GetServiceTypeModel } from "../../../services/Service Type/ServiceTypeApi";
import { AddUpdateServiceAggregatorType, GetServiceAggregatorTypeModel } from "../../../services/Service Aggregator Type/ServiceAggregatorTypeApi";
...
const AddUpdateForm = ({ onClose, modelRequestData }) => {
    ...
    const validate = () => {
        const newErrors = {};
        // Service Aggregator Type 
        if (!formData.serviceAggregatorType || !formData.serviceAggregatorType.trim()) {
            newErrors.serviceAggregatorType = "Service Aggregator Type is required";
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await AddUpdateServiceAggregatorType({ ...formData, userKeyID });
    ...
    const GetModelData = async (KeyID) => {
        const data = await GetServiceAggregatorTypeModel(KeyID);
        if (data.statusCode === 200) {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 p-4">
            <div className="bg-white rounded-2xl flex flex-col shadow-lg text-gray-700 text-base w-full sm:w-80 md:w-96 lg:w-96 xl:w-96 max-h-[90vh]">
                ...
                <div className="flex justify-between bg-background px-6 py-4 border-b border-gray-200 sticky top-0 z-10 rounded-t-2xl">
                    <h2 className="text-xl font-semibold">{modelRequestData?.Action == "Update" ? "Update Aggregator Type" : "Add Aggregator Type"}</h2>
                </div>
                ...
                >
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            name="serviceAggregatorType"
                            placeholder="Enter Service Aggregator Type"
                            value={formData.serviceAggregatorType}
                            onChange={handleChange}
                            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-1 ${errors.serviceAggregatorType ? "border-red-500 focus:ring-red-500" : "focus:ring-secondary"}`}
                        />
                        {errors.serviceAggregatorType && <p className="text-red-500 text-sm mt-1">{errors.serviceAggregatorType}</p>}
                    </div>
                </form>
            ...
            <SuccessModal
                ...
                actionMessage={'Service Aggregator Type'}
            />
