...
import { ChangeAlertStatus, GetAlertList } from "../../../services/Alert/AlertApi";
import BackButton from "../../../components/custom/BackButton";
import { GetAllAlertTypeLookup } from "../../../services/List/AllLookupList";
/* ---------------- Main Component ---------------- */
const ListComponent = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setIsLoading(true);
            try {
                const res = await GetAlertList(appliedFilter);
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                const res = await GetAllAlertTypeLookup();

                if (!isMounted) return;

                if (res?.statusCode === 200) {
                    const options = (res.responseData?.data || []).map(item => ({
                        label: item.alertType, // adjust key if needed
                        value: item.alertTypeID
                    }));

                    setSelectedFilters(prev => ({
                        ...prev,
                        alertTypeLookupList: options
                    }));
                }
            } catch (err) {
    ...
    const handleUpdateStatus = async () => {
        if (modelAction === "Status") {
            try {
                let data = await ChangeAlertStatus(modelRequestData.alertKeyID, modelRequestData.appLangID);
                ...
                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                 ...
                 focus:outline-none focus:ring-1 focus:ring-secondary text-sm"
                    ...
                    <div className="w-[120px] md:w-48 text-sm">
                        <Select
                            options={selectedFilters.alertTypeLookupList}
                            isClearable
                            placeholder="Select Alert type"
                            value={
                                selectedFilters.alertTypeLookupList.find(
                                    opt => opt.value === appliedFilter.alertTypeID
                                ) || null
                            }
                            onChange={(option) => {
                                ...
                                setAppliedFilter(prev => ({
                                    ...prev,
                                    alertTypeID: option?.value,  // this will be null / "Approved" / "Pending" / "Rejected"
                                    pageNo: 0
                                }));
                            }}
            ...
            <div className="flex-1 overflow-auto">
                <table className="min-w-full text-sm text-gray-800 border-collapse">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                        <tr>
                            {["Sr. No.", "Area", "Alert Message", "Alert Type", "Language", "Date", "Status", "Action"].map((h) => (
                                <th
                                    key={h}
                        ...
                        {isLoading ? (
                            ...
                            tableRows.map((u, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 text-gray-700">{u.areaName}</td>
                                    <td className="px-4 py-2 text-gray-700"><Tooltip title={u.alertMessage} arrow>{truncateText(u.alertMessage, 15)}</Tooltip></td>
                                    <td className="px-4 py-2 text-gray-700">{u.alertType}</td>
            ...
            {/* User Form Modal */}
            {
                modalOpen && (
                    <AddUpdateForm
                        ...
                        keyID={state.alertKeyID} //parent key id for add update
